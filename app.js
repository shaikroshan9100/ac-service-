...
import { getAuth, setPersistence, browserLocalPersistence, signInWithEmailAndPassword, createUserWithEmailAndPassword, signOut, onAuthStateChanged } from 'https://www.gstatic.com/firebasejs/12.19.0/firebase-auth.js';
...
// Change this to Nabi's Firebase Authentication email after creating his owner account.
// This is an extra UI check; Firebase Database Rules must also restrict admin access.
const OWNER_EMAIL='';
...
$('bookingForm').onsubmit=async e=>{
  ...
  try{
    ...
    if(!user || currentRole!=='customer') return toast('Please login as a customer before booking.');
    b.uid=user.uid;
    const updates={};
    updates['bookings/'+b.id]=b;
    updates['userBookings/'+user.uid+'/'+b.id]=b;
...
window.lookupBookings=function(){
  ...
  let a=[];
  if(user && currentRole==='customer') a=cloudBookings.filter(x=>x.uid===user.uid || x.phone===phone);
  else a=getLocal().filter(x=>x.phone===phone);
  $('bookingsList').innerHTML=a.length?a.map(b=>`<div class="booking"><div class="bookingHead"><div><h3>${b.service}</h3><small>${b.id}</small></div><span class="status">${b.status}</span></div><p>📅 ${b.date} • ${b.time}</p><p>❄️ ${b.acType} • ${b.brand} • ${b.count} AC</p><p>📍 ${b.address}</p><p>💳 ${b.payment} • ${b.amount?'Starting ₹'+b.amount:'Quote after inspection'}</p><p>👨‍🔧 ${b.technician}</p><div class="actions"><a class="smallBtn" target="_blank" href="https://wa.me/919505225027?text=Hello%20AC%20WORKS,%20my%20booking%20ID%20is%20${b.id}.">WhatsApp</a><a class="smallBtn" href="tel:9505225027">Call</a></div></div>`).join(''):`<div class="booking"><h3>No booking found</h3><p>${user?'Your bookings will appear here.':'Enter your mobile number or login to view your bookings.'}</p></div>`;
};

window.customerLogin=async function(){
  const email=$('customerEmail').value.trim().toLowerCase(), pass=$('customerPassword').value;
  if(!email||!pass)return toast('Enter your email and password.');
  const btn=$('customerLoginBtn'); if(btn){btn.disabled=true;btn.textContent='Logging in...';}
  try{
    await setPersistence(auth,browserLocalPersistence);
    const cred=await signInWithEmailAndPassword(auth,email,pass);
    const roleSnap=await get(ref(db,'users/'+cred.user.uid+'/role'));
    if(roleSnap.val()!=='customer'){await signOut(auth);throw {code:'auth/not-customer'};}
    toast('Customer login successful.');
  }catch(err){const m={'auth/invalid-credential':'Wrong email or password.','auth/invalid-email':'Enter a valid email address.','auth/user-disabled':'This account is disabled.','auth/too-many-requests':'Too many attempts. Try later.','auth/not-customer':'This is not a customer account.'};toast(m[err?.code]||('Login failed: '+(err?.code||'unknown error')));}finally{if(btn){btn.disabled=false;btn.textContent='Customer Login';}}
};
window.customerSignup=async function(){
  const email=$('customerEmail').value.trim().toLowerCase(), pass=$('customerPassword').value, name=$('customerName').value.trim();
  if(!name||!email||pass.length<6)return toast('Enter name, valid email and password (6+ characters).');
  const btn=$('customerSignupBtn'); if(btn){btn.disabled=true;btn.textContent='Creating...';}
  try{
    await setPersistence(auth,browserLocalPersistence);
    const cred=await createUserWithEmailAndPassword(auth,email,pass);
    await set(ref(db,'users/'+cred.user.uid),{role:'customer',name,email,createdAt:new Date().toISOString()});
    toast('Customer account created.');
  }catch(err){const m={'auth/email-already-in-use':'Email already registered. Use Customer Login.','auth/invalid-email':'Enter a valid email address.','auth/weak-password':'Password must be at least 6 characters.'};toast(m[err?.code]||('Signup failed: '+(err?.code||'unknown error')));}finally{if(btn){btn.disabled=false;btn.textContent='Create Customer Account';}}
};
window.customerLogout=async function(){await signOut(auth);toast('Logged out.');window.go('home');};

window.adminLogin=async function(){
  const email=$('adminEmail').value.trim().toLowerCase();
  const pass=$('adminPassword').value;
  if(!email||!pass){toast('Enter Nabi\'s admin email and password.');return;}
  if(OWNER_EMAIL && email!==OWNER_EMAIL.toLowerCase()){toast('This account is not the AC WORKS owner account.');return;}
  const button=document.querySelector('#adminLogin .fullBtn');
  ...
  try{
    ...
    const messages={
      ...
      'auth/not-admin':'This account is not the AC WORKS owner account.'
    };
    toast(messages[code]||('Login failed: '+code));
  }finally{
    if(button){button.disabled=false;button.textContent='Login to Admin';}
  }
};
window.adminLogout=async function(){await signOut(auth);$('adminEmail').value='';$('adminPassword').value='';toast('Logged out.');go('home');};

function subscribeBookings(){
  if(unsubscribeBookings)unsubscribeBookings();
  const path=currentRole==='admin'?'bookings':(currentRole==='customer'?'userBookings/'+auth.currentUser.uid:null);
  if(!path){cloudBookings=[];return;}
  unsubscribeBookings=onValue(ref(db,path),snap=>{
    const data=snap.val()||{};
    cloudBookings=Object.values(data).sort((a,b)=>(b.createdAt||'').localeCompare(a.createdAt||''));
    if(currentRole==='admin') saveLocal(cloudBookings);
    if(currentRole==='customer'){
      cloudBookings.forEach(b=>{const a=getLocal().filter(x=>x.id!==b.id);a.unshift(b);saveLocal(a);});
      lookupBookings();
    }
    renderAdmin();
  },err=>{console.error(err);toast('Could not read bookings. Check Firebase rules.');});
}
function renderAdmin(){
  const user=auth.currentUser;
  $('adminLogin').classList.toggle('hidden',currentRole==='admin');
...
window.setStatus=async function(id,status){
  if(currentRole!=='admin')return toast('Owner access required.');
  ...
  try{await update(ref(db),{'bookings/'+id+'/status':status,'bookings/'+id+'/technician':patch.technician||b.technician,'userBookings/'+b.uid+'/'+id+'/status':status,'userBookings/'+b.uid+'/'+id+'/technician':patch.technician||b.technician});toast(id+' → '+status);}catch(err){console.error(err);toast('Update failed.');}
};
...
async function setupForegroundNotifications(){
  try{
    ...
    onMessage(messaging,payload=>{
      ...
      const body=payload.notification?.body||'A new booking has arrived.';
      if('Notification' in window && Notification.permission==='granted') new Notification(title,{body,icon:'ac-works-poster.png'});
      toast(body);
    });
  }catch(e){console.warn('Foreground notification setup skipped',e);}
}
...
onAuthStateChanged(auth,async user=>{
  ...
  if(user){
    try{
      ...
      currentRole=snap.val()||'customer';
    }catch(e){currentRole='customer';}
    subscribeBookings();
    renderAdmin();
    if(currentRole==='admin') window.go('admin');
    else window.go('home');
  }else{
    currentRole='guest';
    renderAdmin();
...
setPersistence(auth,browserLocalPersistence).catch(err=>console.error('Auth persistence setup failed',err));
